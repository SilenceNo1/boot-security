...
public class TemplateUtil {
    ...
    private static String getFields(List<String> beanFieldName, List<String> beanFieldType, List<String> beanFieldValue, List<String> beanFieldComment) {
    ...
    private static String getset(List<String> beanFieldName, List<String> beanFieldType) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            buffer.append("\t}\n");
            buffer.append("\tpublic void set").append(StringUtils.substring(name, 0, 1).toUpperCase() + name.substring(1, name.length())).append("(").append(type).append(" ")
                    .append(name).append(") {\n");
    ...
    private static String getWhere(List<String> columnNames, List<String> beanFieldName) {
        ...
        for (int i = 0; i < size; i++) {
            String column = columnNames.get(i);
            String fieldName = beanFieldName.get(i);
            buffer.append("\t\t\t<if test=\"params." + fieldName + " != null and params." + fieldName + " != ''\">\n");
            buffer.append("\t\t\t\tand " + column).append(" = ").append("#{params.").append(fieldName).append("} \n");
            buffer.append("\t\t\t</if>\n");
        }
    ...
    public static void saveHtmlList(GenerateInput input) {
        ...
        List<String> beanFieldComment = input.getBeanFieldComment();
        text = text.replace("{ths}", getHtmlThs(beanFieldComment));
        ...
        text = text.replace("{beanParamName}", beanParamName);
        text = text.replace("{addDivs}", getAddDivs(beanFieldNames, beanFieldComment, beanFieldValue));
    ...
    private static String getAddDivs(List<String> beanFieldNames, List<String> beanFieldComment, List<String> beanFieldValue) {
        StringBuilder builder = new StringBuilder();
        for (int i = 0; i < beanFieldNames.size(); i++) {
            String b = beanFieldNames.get(i);
            String c = beanFieldComment.get(i);
            String value = beanFieldValue.get(i);
            if (!"id".equals(b) && !"createTime".equals(b) && !"updateTime".equals(b)) {
                builder.append("\t\t\t<div class='form-group'>\n");
                builder.append("\t\t\t\t<label class='col-md-2 control-label'>" + c + "</label>\n");
                builder.append("\t\t\t\t<div class='col-md-10'>\n");
                builder.append("\t\t\t\t\t<input class='form-control' placeholder='" + b + "' type='text' name='" + b + "' id='" + b + "' value='" + value
                        + "' data-bv-notempty='true' data-bv-notempty-message='" + c + " 不能为空'>\n");
                builder.append("\t\t\t\t</div>\n");
                builder.append("\t\t\t</div>\n");
            }
        }
